...
如果两个人之间编辑一个共享文件，先进行编辑的人会被后进行编辑的内容覆盖 git版本控制系统就是为了解决这一问题诞生的

###1.1.2 git的概念

git是目前世界上最先进的“分布式”版本控制系统，他可以在任何时间把文档的状态作为更行记录保存起来。因此，可以把编辑过得文档“复原”到以前的状态，也可以显示编辑前后的内容差异
...
所谓托管平台，就是可以存放文件，只不过上边的文件是可以被公开访问的
...
差异记录，就是提交状态的时候（描述或记录）记录的内容，执行提交的时候，系统会要求输入提交信息，这是一定要提交的，否则会提示执行失败或提交失败
